...
function EditItem() {
    ...
    useEffect(() => {
        AxiosInstance.get(`api/item/get-item/${id}`).then((res) => {
            if (res.data.success) {
                setCuurentItem(res.data.item)
            }
        }).catch((err) => {
            console.log(err)
        })
    ...
    useEffect(() => {
        ...
        setImgUrl(currentitem?.image || null);
    }, [currentitem]);
    ...
    const sumithandel = (e) => {
        ...
        AxiosInstance.put(`api/item/item-edit/${id}`, formData).then((res) => {
            if (res.data.success) {
